...
def get_grupos_completos(_self) -> pd.DataFrame:
    """Obtiene grupos con información de acción formativa desde la vista optimizada."""
    try:
        query = _self.supabase.table("vista_grupos_completos").select("*")
        query = _self._apply_empresa_filter(query, "vista_grupos_completos")
...
def get_participantes_completos(_self) -> pd.DataFrame:
    ...
    try:
        ...
        if "grupo" in df.columns:
            ...
            # Garantizar columnas aunque no haya relación
            df["grupo_codigo"] = ""
            df["grupo_id"] = None

        # Aplanar relaciones de empresa
