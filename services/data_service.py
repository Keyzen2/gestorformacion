...
class DataService:
    ...
    def get_participantes_para_formulario(_self, rol: str, empresa_id_gestor: str = None) -> pd.DataFrame:
        ...
        try:
            # Obtener datos básicos usando el método existente
            ...
            if rol == "admin":
                # Para admin: campo empresa_sel editable (basado en empresa_nombre)
                df["empresa_sel"] = df.get("empresa_nombre", "")
            elif rol == "gestor":
                # Para gestor: campo empresa_asignada readonly
                # Si no tienen empresa_nombre, buscar en el diccionario
                if "empresa_nombre" in df.columns and not df["empresa_nombre"].isna().all():
                    df["empresa_asignada"] = df["empresa_nombre"]
                else:
                    # Fallback: buscar nombre de la empresa del gestor
                    try:
...
# (con 4 espacios de indentación desde el margen izquierdo)
    ...
    def create_tutor_grupo(_self, grupo_id: str, tutor_id: str) -> bool:
        """Asigna un tutor a un grupo."""
        try:
            _self.supabase.table("tutores_grupos").insert({
                "grupo_id": grupo_id,
                "tutor_id": tutor_id,
                "created_at": datetime.utcnow().isoformat()
            }).execute()
    ...
    def delete_tutor_grupo(_self, relacion_id: str) -> bool:
        """Elimina la relación tutor-grupo."""
        try:
            _self.supabase.table("tutores_grupos").delete().eq("id", relacion_id).execute()
            return True
        except Exception as e:
    ...
    def calcular_limite_fundae(_self, modalidad: str, horas: int, participantes: int) -> tuple:
    ...
    def limpiar_cache_grupos(_self):
        """Limpia cache relacionado con grupos - FUNCIÓN CRÍTICA."""
        try:
            # Limpiar todos los caches relacionados con grupos
            if hasattr(_self, 'get_grupos_completos'):
                _self.get_grupos_completos.clear()
            if hasattr(_self, 'get_participantes_completos'):
                _self.get_participantes_completos.clear()
            if hasattr(_self, 'get_tutores_completos'):
                _self.get_tutores_completos.clear()
            if hasattr(_self, 'get_acciones_formativas'):
                _self.get_acciones_formativas.clear()
            if hasattr(_self, 'get_empresas'):
                _self.get_empresas.clear()
            if hasattr(_self, 'get_empresas_con_modulos'):
                _self.get_empresas_con_modulos.clear()
            # Caches específicos de grupos
            if hasattr(_self, 'get_grupo_costes'):
                _self.get_grupo_costes.clear()
            if hasattr(_self, 'get_grupo_bonificaciones'):
                _self.get_grupo_bonificaciones.clear()
            if hasattr(_self, 'get_tutores_grupo'):
                _self.get_tutores_grupo.clear()
            if hasattr(_self, 'get_empresas_grupo'):
                _self.get_empresas_grupo.clear()
        except Exception as e:
