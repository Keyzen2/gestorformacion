...
class DataService:
    ...
    def get_metricas_empresas(_self) -> Dict[str, Any]:
        ...
        try:
            ...
            modulos_cols = [
                ...
                "docu_avanzada_activo", "crm_activo"
            ]
    ...
    def update_empresa(_self, empresa_id: str, datos_empresa: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            update_data = {k: v for k, v in datos_empresa.items() if k != "crm_activo" and k != "crm_inicio" and k != "crm_fin"}
            if update_data:
    ...
    def get_grupos_completos(_self) -> pd.DataFrame:
        ...
        try:
            ...
            if "accion" in df.columns:
                df["accion_nombre"] = df["accion"].apply(
                    lambda x: x.get("nombre") if isinstance(x, dict) else ""
                )
    ...
    def get_participantes_completos(_self) -> pd.DataFrame:
        ...
        try:
            ...
            if "grupo" in df.columns:
                df["codigo_grupo"] = df["grupo"].apply(
    ...
    def get_ajustes_app(_self, campos: Optional[str] = None) -> Dict[str, Any]:
        """Obtiene ajustes de la aplicación."""
        try:
    ...
    def get_metricas_dashboard(_self) -> Dict[str, Any]:
        """Obtiene métricas para el dashboard principal."""
        try:
            metricas = {}
            
            # Total empresas
            empresas_df = _self.get_empresas()
            metricas["total_empresas"] = len(empresas_df) if not empresas_df.empty else 0
            
            # Total usuarios
            usuarios_df = _self.get_usuarios(include_empresa=False)
            metricas["total_usuarios"] = len(usuarios_df) if not usuarios_df.empty else 0
            
            # Total grupos
            grupos_df = _self.get_grupos_completos()
            metricas["total_grupos"] = len(grupos_df) if not grupos_df.empty else 0
            
            # Total participantes
            participantes_df = _self.get_participantes_completos()
            metricas["total_participantes"] = len(participantes_df) if not participantes_df.empty else 0
            
            # Participantes por grupo (promedio)
            if not grupos_df.empty and metricas["total_grupos"] > 0:
                metricas["participantes_por_grupo"] = round(metricas["total_participantes"] / metricas["total_grupos"], 1)
            else:
                metricas["participantes_por_grupo"] = 0
            
            return metricas
            
        except Exception as e:
            st.error(f"⚠️ Error al calcular métricas del dashboard: {e}")
            return {
                "total_empresas": 0,
                "total_usuarios": 0,
                "total_grupos": 0,
                "total_participantes": 0,
                "participantes_por_grupo": 0
            }
