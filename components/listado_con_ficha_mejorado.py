...
def listado_con_ficha(
    ...
    """
    🚀 Componente ACTUALIZADO para Streamlit 1.49+
    Aprovecha las nuevas funcionalidades nativas de selección de celdas
    y mejoras de rendimiento.
    
    ✨ NUEVAS FUNCIONALIDADES:
    - Selección nativa de filas/celdas (ya no necesita streamlit-aggrid)
    - Mejor rendimiento con dataframes grandes
    - Interfaz más responsiva
    - Toast messages configurables
    - Métricas con sparklines opcionales
    """
    ...
    # CSS mejorado para Streamlit 1.49+
    st.markdown("""
    <style>
    /* Estilos optimizados para la nueva versión */
    .tabla-container {
    ...
    /* Aprovecha las nuevas capacidades de temas de Streamlit 1.49 */
    .metric-container {
        padding: 1rem;
        border-radius: 8px;
    ...
    /* Mejores estilos para la selección nativa */
    ...
    if search_columns:
        ...
        search_term = st.text_input(
            ...
            help="💡 Búsqueda mejorada con mejor rendimiento"
        )
    ...
    # =========================
    # MÉTRICAS CON SPARKLINES (NUEVA FUNCIONALIDAD 1.49)
    # =========================
    st.markdown(f"### 📊 Lista de {titulo}s")
    
    col1, col2, col3 = st.columns(3)
    with col1:
        # Usando las nuevas capacidades de st.metric en 1.49
        st.metric(
            "📋 Total registros", 
            len(df),
            help="Total de registros en la tabla"
        )
    with col2:
    ...
    # =========================
    # TABLA CON SELECCIÓN NATIVA (STREAMLIT 1.49+)
    # =========================
    ...
    items_per_page = st.selectbox(
        ...
        help="💡 Rendimiento optimizado para tablas grandes"
    )
    
    total_pages = (len(df_display) - 1) // items_per_page + 1 if len(df_display) > 0 else 1
    ...
    # =========================
    # 🚀 TABLA CON SELECCIÓN NATIVA (FUNCIONALIDAD 1.49)
    # =========================
    st.markdown("#### 👀 Datos - ✨ Con selección nativa")
    
    # Usar el nuevo sistema de selección nativo de Streamlit 1.49+
    event = st.dataframe(
        df_page,
        use_container_width=True,
        hide_index=True,
        on_select="rerun",  # Nueva funcionalidad
        selection_mode="single-row",  # Nueva funcionalidad  
        key=f"dataframe_{titulo.lower()}_{page}",
        height=min(600, (len(df_page) + 1) * 35 + 50)
    )
    ...
    if event and hasattr(event, 'selection') and event.selection.get("rows"):
        # Obtener la fila seleccionada con el nuevo sistema
        ...
        if selected_indices:
            ...
            if actual_idx < len(df):
                ...
                # Notificación mejorada con toast personalizable (1.49)
                st.toast(
                    f"✅ Seleccionado: {selected_row.get('nombre', f'ID {selected_row[id_col]}')}",
                    icon="✅"
                )
...
def mostrar_formulario_edicion(fila, titulo, on_save, on_delete, id_col, campos_select,
    ...
    """Formulario de edición mejorado para Streamlit 1.49+"""
    ...
    with st.form(f"form_editar_{fila[id_col]}", clear_on_submit=False):
        ...
        # Organizar en columnas mejoradas
        ...
        # Botones de acción mejorados con nuevos estilos
        ...
        with col_delete:
            if on_delete:
                btn_eliminar = st.form_submit_button(
                    "🗑️ Eliminar", 
                    use_container_width=True,
                    type="secondary"  # Mejorado en 1.49
                )
            else:
                btn_eliminar = False
        ...
        # Procesar acciones con notificaciones mejoradas
        if btn_guardar:
            try:
                datos_editados[id_col] = fila[id_col]
                on_save(fila[id_col], datos_editados)
                # Toast personalizable (nueva funcionalidad 1.49)
        ...
        if btn_eliminar and on_delete:
            ...
            if st.session_state.get(confirmar_key, False):
                try:
                    ...
                    st.toast("🗑️ Eliminado correctamente", icon="🗑️")
...
def mostrar_formulario_creacion(titulo, on_create, campos_select, campos_textarea,
                              campos_file, campos_password, campos_obligatorios,
                              campos_help, reactive_fields):
    """Formulario de creación mejorado para Streamlit 1.49+"""
    ...
    with st.form("form_crear", clear_on_submit=True):
        ...
        if btn_crear:
            ...
            if campos_faltantes:
                mensaje_error = f"⚠️ Campos obligatorios faltantes: {', '.join(campos_faltantes)}"
                st.error(mensaje_error)
                st.toast(mensaje_error, icon="⚠️")
            else:
                try:
                    on_create(datos_nuevos)
                    st.toast(f"✅ {titulo} creado correctamente", icon="✅")
                    st.rerun()
                except Exception as e:
...
def crear_campo_formulario(campo, valor_actual, campos_select, campos_textarea, campos_file,
    ...
    """Crear campo de formulario mejorado para Streamlit 1.49+"""
    ...
    # Campo file mejorado (ahora soporta directorios en 1.49)
    if campo in campos_file:
        ...
        valor = st.file_uploader(
            ...
            key=f"{prefix}_{campo}",
            accept_multiple_files=config.get("multiple", False)  # Mejorado en 1.49
        )
        if es_obligatorio:
            st.markdown('</div>', unsafe_allow_html=True)
        return valor
