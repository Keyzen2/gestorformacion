# =========================
# SECCIONES ADICIONALES (Solo para grupos existentes) - MANTENER LÓGICA ORIGINAL
# =========================
...
def mostrar_seccion_centro_gestor(grupos_service, grupo_id):
    ...
    try:
        ...
        if modalidad_norm in ["TELEFORMACION", "MIXTA"]:
            ...
            st.info("Esta sección solo es necesaria para modalidades Teleformación o Mixta")
                        ...
                        for participante_str in participantes_quitar:
                            try:
                                ...
                                if not participante_row.empty:
                                    participante_id = participante_row.iloc[0]["relacion_id"]
...
def main(supabase, session_state):
    """Función principal de gestión de grupos con mejoras visuales de Streamlit 1.49."""
    ...
    # Botón para crear nuevo grupo con modal
    ...
    with col2:
        if st.button(
            ...
            type="primary", 
            use_container_width=True,
            help="Abrir formulario para crear un nuevo grupo FUNDAE"
        ):
            modal_crear_grupo(grupos_service)
    
    # Tabla principal de grupos mejorada
    st.markdown("### 📊 Listado de Grupos")
    
    if df_grupos.empty:
        st.info("📋 No hay grupos registrados. Crea tu primer grupo usando el botón superior.")
    else:
        ...
        # Mostrar tabla con selección mejorada
        event = st.dataframe(
            ...
            column_config={
                ...
                "fecha_fin_prevista": st.column_config.DateColumn(
                    "Fecha Fin Prevista",
                    format="DD/MM/YYYY"
                )
            }
    ...
    if hasattr(st.session_state, 'grupo_seleccionado'):
        if st.session_state.grupo_seleccionado == "nuevo":
            ...
            if grupo_id:
                st.divider()
                mostrar_secciones_adicionales(grupos_service, grupo_id)import streamlit as st
import pandas as pd
...
# =========================
# COMPONENTES UI MEJORADOS - STREAMLIT 1.49
# =========================

def mostrar_kpis_grupos(df_grupos):
    """Muestra KPIs de grupos con métricas mejoradas de Streamlit 1.49."""
    if df_grupos.empty:
    ...
    with col2:
        # Calcular porcentaje y delta visual
...
def mostrar_avisos_grupos(grupos_pendientes):
    """Muestra avisos de grupos pendientes de finalización con alertas mejoradas."""
    if not grupos_pendientes:
        return
    
    # Alerta principal mejorada
    st.warning(
        f"⚠️ **{len(grupos_pendientes)} grupo(s) pendiente(s) de finalización**",
        icon="⚠️"
    )
    
    # Expandible con información detallada
    with st.expander("Ver grupos pendientes de finalización", expanded=False):
        for i, grupo in enumerate(grupos_pendientes[:5]):  # Mostrar máximo 5
            with st.container():
                col1, col2 = st.columns([3, 1])
                
                with col1:
                    # Información del grupo con mejor formato
                ...
                with col2:
                    ...
                    if st.button(
                        "🏁 Finalizar", 
                        key=f"finalizar_{grupo.get('id')}", 
                        type="secondary",
                        use_container_width=True,
                        help="Marcar grupo como finalizado"
                    ):
...
def crear_selector_horario_fundae(prefix=""):
    ...
    # Opción de configuración con mejor UI
    ...
    # Tramo mañana con mejor formato
        ...
        with col1:
            ...
            with sub_col1:
                manana_inicio = st.selectbox(
                    ...
                    help="Hora de inicio del tramo matinal"
                )
            with sub_col2:
                if manana_inicio:
                    ...
                    manana_fin = st.selectbox(
                        ...
                        help="Hora de finalización del tramo matinal"
                    )
    
    # Tramo tarde con mejor formato
        ...
        with col2:
            ...
            with sub_col1:
                tarde_inicio = st.selectbox(
                    ...
                    help="Hora de inicio del tramo vespertino"
                )
            with sub_col2:
                if tarde_inicio:
                    # Filtrar horas fin que sean posteriores al inicio
                    ...
                    tarde_fin = st.selectbox(
                        ...
                        help="Hora de finalización del tramo vespertino"
                    )
    
    # Días de la semana con mejor visualización
    ...
    for i, (dia_corto, dia_largo) in enumerate(zip(DIAS_SEMANA, NOMBRES_DIAS)):
        with cols[i]:
            # Valor por defecto L-V
            valor_defecto = dia_corto in ["L", "M", "X", "J", "V"]
            if st.checkbox(
                f"{dia_corto}\n{dia_largo[:3]}",
                value=valor_defecto,
                key=f"{prefix}_dia_{dia_corto}",
                help=f"Incluir {dia_largo} en el horario"
            ):
...
def modal_crear_grupo(grupos_service):
    """Modal moderno para crear grupos con mejor UX."""
    ...
    with st.form("form_crear_grupo_modal", clear_on_submit=False):
        # Datos básicos con mejor organización
        ...
        with col2:
            ...
            st.text_input(
                ...
                disabled=True,
                help="Modalidad derivada de la acción formativa"
            )
        
        # Localización con selectores
        ...
        with col4:
            ...
            if provincia_sel:
                ...
                localidad_sel = st.text_input("Localidad *", help="Especifique la localidad")
        
        # Información adicional
        col5, col6 = st.columns(2)
        with col5:
            cp = st.text_input("Código Postal", help="Código postal de impartición")
            responsable = st.text_input("Responsable *", help="Persona responsable del grupo")
        
        with col6:
            telefono_contacto = st.text_input("Teléfono *", help="Teléfono de contacto")
            lugar_imparticion = st.text_area("Lugar", height=60, help="Lugar donde se impartirá")
        ...
        # Observaciones
        observaciones = st.text_area("Observaciones", height=80, help="Información adicional")
...
def mostrar_formulario_grupo(grupos_service, grupo_seleccionado=None, es_creacion=False):
    """Formulario unificado para crear/editar grupos - MANTENER LÓGICA ORIGINAL."""
    ...
    if es_creacion:
        ...
        # Mostrar estado actual con mejor formato
        color_estado = {"ABIERTO": "🟢", "FINALIZAR": "🟡", "FINALIZADO": "✅"}
        st.info(f"Estado actual: {color_estado.get(estado_actual, '⚪')} **{estado_actual}**", icon=color_estado.get(estado_actual, '⚪'))
    
    # RESTO DE LA LÓGICA DEL FORMULARIO ORIGINAL
    # =====================
    ...
    with st.expander("📋 1. Datos Básicos FUNDAE", expanded=True):
        ...
        with col2:
            ...
            # Manejar valores None y 0 correctamente
        ...
        # =====================
        # Mejorar lógica de cuándo mostrar finalización
